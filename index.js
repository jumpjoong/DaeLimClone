...
const color = document.querySelectorAll('.log-effects li')
const ffff = document.querySelector('.abc li:nth-of-type(1)')
const gggg = document.querySelector('.abc li:nth-of-type(2)')

let fff = 0;
color.forEach(function(btn, key) {
  btn.addEventListener('click', ()=> {
    color[fff].classList.remove('on')
    color[key].classList.add('on')
    fff = key
  })
  ...
  color[0].onclick = function() {
    ffff.innerHTML = "개인 아이디 찾기"
    gggg.innerHTML = "개인 비밀번호 찾기"
  }
  color[1].onclick = function() {
    ffff.innerHTML = "기업 아이디 찾기"
    gggg.innerHTML = "기업 비밀번호 찾기"
  }
})

const bbc = document.querySelector('.topBtn')

bbc.addEventListener('click', function(e) {
  e.scrollTo(0, 0)
})
